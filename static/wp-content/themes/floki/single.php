...
<?php
//Special MacG By Raph
	$site_origine=$_SERVER['HTTP_REFERER'];
	if( ! empty($site_origine) ) {
		if ( parse_url($site_origine, PHP_URL_HOST) == 'forums.macg.co' ||
		     parse_url($site_origine, PHP_URL_HOST) == 'macg.co' ||
		     parse_url($site_origine, PHP_URL_HOST) == 'igen.fr' ) {
			?>
			<div style="">Chers lecteurs de Mac Génération.</div>
			<?php
		}
	}
...
<?php echo parse_url($site_origine, PHP_URL_HOST); ?>
<?php get_footer(); ?>
